...
{
    class BDCRA_Heu : RoutingStrategy
    {
        private static readonly double MaxValue = double.MaxValue;

        public BDCRA_Heu(Topology topology)
            : base(topology)
        {
            _Topology = topology;
        }
        ...
        public override List<Link> GetPath(Request request)
        {
            ...
            foreach (var link in _Topology.Links)
            {
                ...
                //w1[link.Key] = (link.UsingBandwidth * Math.Pow(10, 8)) / link.Capacity; // caot & Dung khung
