...
namespace NetworkSimulator.RoutingComponents.RoutingStrategies
{
    class MDMF_Heu : RoutingStrategy
    {
        ...
        public override List<Link> GetPath(Request request)
        {
            ...
            HashSet<string> eliminatedLinks_tmp = new HashSet<string>(eliminatedBwLinks.Select(o => o.Key));
            eHRABDC ehrabdc = new eHRABDC(_Topology);

            // 14/07/21 caoth: ehe
            //var path = FindOptimalPath(
                //_Topology, E, request.SourceId, request.DestinationId, w1, w2, request.Delay);
            var tempPath = ehrabdc.FindOptimalPath(
                _Topology, eliminatedLinks_tmp, request.SourceId, request.DestinationId, w1, w2, request.Delay);
            var path = tempPath;
